...
function filter() {
  ...
  cards.forEach((el) => {
    let title = el.querySelector('.card__title').textContent.toLowerCase();
...
function doMath(){
  ...
  document.getElementById("price2") =  a * b;
...
(function() {
    ...
    function getCartItemPrices() {
      ...
      let nums = cartContent.querySelectorAll("tr td:nth-child(4)");
      let nums2 = cartContent.querySelectorAll("tr td:nth-child(3)");
      ...
      if (nums.length > 0) {
        for (let cell = 0; cell < nums.length; cell++) {
          ...
          let num2 = nums2[cell].innerText;
          num = num.replace(/[^\d]/g, "");
          num = parseFloat(num);
          prices.push(num*num2/100);
        }
    ...
    function displayProducts() {
      ...
      if (lsContent !== null) {
        for (let product of lsContent) {
          productMarkup += `
            <tr>
            <td><img class="cart-image" src="${product.image}"
          }" width="120"></td>
            <td>
              ${product.name}
            </td>
    ...
    function saveProduct(clickedBtn) {
      ...
      const prodQuant = cardInfo.querySelector("input").value;
      const prodTotal = prodPrice * prodQuant;
